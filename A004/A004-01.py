import logging
logging.basicConfig(filename='A004-01.log', encoding='utf-8', level=logging.DEBUG)
import pandas as pd
...
def APIdata(tickerNum: int): #DataSet historical[1],  
    historicalClose = list(yf.download(currency[tickerNum] + '-USD'
                 ,period='max', interval='1d', progress=False)['Close'])
...
proccesModel = model.methodSPC(APIdata(0), interval, futureForcast)
...
#loop list
n_result = {'SumPerChg':[], 'predicted': [], 'FutureChg':[], 'accuracy':[]}
...
for countARRdata in range(0, len(ARRdata)):
    ...
    try: n_result['accuracy'].append(float(predicted)/float(ARRdata[countARRdata][3]))
    except: n_result['accuracy'].append(None)
...
predictedPrice = APIdata(0)[0]+(n_result['predicted'][0]/100*APIdata(0)[0])
